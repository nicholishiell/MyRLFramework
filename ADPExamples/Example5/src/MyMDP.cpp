...
MyMDP::MyMDP()
{
    ...
    for(int i = 0; i < 150; i++)
    {
        decisionHistory_[i] = {0,0,0,0};
        stateHistory_[i] = {0,0,0,0};
    }
}
...
{
    const auto dArray = std::dynamic_pointer_cast<MyDecision>(decision)->GetDecisionArray();
    const auto sArray = std::dynamic_pointer_cast<MyState>(state)->GetTriagePopulations();
...
MyMDP::UpdateDecision()
{
    ...
    {
        if(decisionArray[i] <=  population[i])
